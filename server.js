...
import { startScheduler, updatePrinters, stopScheduler } from './scheduler.js';
...
// Accepts either a Supabase row ({id, ...}) or a config.json entry
// ({printer_id, usb_path, ...}) and produces the single shape the rest of the
// server expects. Legacy `usb_path` is aliased to `usb_device_path` so old
// config.json files keep working after upgrade.
function normalizePrinter(p) {
  const connection_type = p.connection_type || 'usb';
  return {
    printer_id: p.printer_id ?? p.id,
    name: p.name,
    connection_type,
    ip_address: p.ip_address ?? null,
    port: connection_type === 'wifi' ? (p.port ?? 9100) : (p.port ?? null),
    usb_device_path: p.usb_device_path ?? p.usb_path ?? null,
    // Informational only. ESC/POS bytes arrive pre-rendered for the
    // printer's paper width — the staff app reads paper_width_mm from
    // Supabase and bakes it into the generated bytes before POSTing
    // (see bkkboost-dine apps/staff build-receipt-escpos.ts and
    // generate-jobs.ts). Kept in the normalized shape for visibility via
    // GET /printers and potential future use.
    paper_width_mm: p.paper_width_mm ?? null,
  };
}

// Returns a list of human-readable change descriptions between two printer
// lists (matched by printer_id). Empty list means the configs are equivalent.
function diffPrinters(oldList, newList) {
  const oldMap = new Map(oldList.map((p) => [p.printer_id, p]));
  const newMap = new Map(newList.map((p) => [p.printer_id, p]));
  const changes = [];

  for (const [id, np] of newMap) {
    const op = oldMap.get(id);
    if (!op) {
      changes.push(`${np.name} added`);
      continue;
    }
    for (const field of ['name', 'connection_type', 'ip_address', 'port', 'usb_device_path', 'paper_width_mm']) {
      if (op[field] !== np[field]) {
        changes.push(`${np.name} ${field} changed ${op[field] ?? 'null'}→${np[field] ?? 'null'}`);
      }
    }
  }
  for (const [id, op] of oldMap) {
    if (!newMap.has(id)) changes.push(`${op.name} removed`);
  }
  return changes;
}

config.printers = (config.printers || []).map(normalizePrinter);
...
async function fetchPrintersFromSupabase() {
  ...
  const url = `${config.supabase_url}/rest/v1/restaurant_printers?restaurant_id=eq.${config.restaurant_id}&is_active=eq.true&select=id,name,connection_type,ip_address,port,usb_device_path,paper_width_mm`;
  ...
  return rows.map(normalizePrinter);
}
...
async function refreshPrinterConfig() {
  try {
    ...
    // Guard against a bad query / deactivated-all / misconfigured restaurant_id
    // wiping the live printer list. Only accept an empty response when we
    // already had nothing loaded.
    if (printers.length === 0 && config.printers.length > 0) {
      logger.error(`Supabase returned 0 printers — keeping current config (possible misconfiguration). Current: ${config.printers.length} printer(s).`);
      return;
    }

    const changes = diffPrinters(config.printers, printers);
    if (changes.length === 0) {
      logger.debug('Printer config unchanged');
      return;
    }

    for (const msg of changes) {
      logger.info(`Printer config updated: ${msg}`);
    }
...
// GET /health — registered BEFORE auth middleware so uptime monitors and
// Cloudflare health probes can hit it without a token.
...
app.post('/print', (req, res) => {
  ...
  // INSERT OR IGNORE + changes() gives atomic idempotency — no check-then-act
  // race between jobExists and enqueue.
  ...
  if (result.changes === 0) {
    logger.info(`Job ${jobId} duplicate POST — already queued for printer ${printer.name}`);
    return res.status(200).json({ jobId, status: 'duplicate' });
  }

  logger.info(`Job ${jobId} queued for printer ${printer.name} (${printer.connection_type})`);
...
app.post('/print-round', async (req, res) => {
  ...
  try {
    ...
    if (!staffRes.ok) {
      ...
      // once Response body is consumed it can't be re-read.
      const rawText = await staffRes.text().catch(() => '<unreadable>');
      let errPayload = {};
      try {
        errPayload = JSON.parse(rawText);
      } catch (parseErr) {
        logger.debug(`Staff app non-JSON error body (status ${staffRes.status}): ${rawText.slice(0, 500)}`);
      }
      logger.error(`Staff app returned ${staffRes.status} for round ${round_id}: ${errPayload.error || 'Unknown error'}`);
      return res.status(502).json({ error: `Staff app error: ${errPayload.error || staffRes.status}` });
    }
...
app.get('/logs', (req, res) => {
  const { status, printer_id, job_type, limit, offset, from, to, q } = req.query;
  ...
  const result = listJobsForLogs({
    ...
    // Free-text search over job id / round_id. Capped so a pathological string
    // cannot build an enormous LIKE pattern.
    q: typeof q === 'string' && q.trim().length > 0 ? q.trim().slice(0, 100) : undefined,
  });
  res.json(result);
});
...
let _supabaseRefreshInterval = null;
...
const httpServer = app.listen(PORT, async () => {
  ...
  try {
    ...
    if (printers) {
      if (printers.length === 0 && config.printers.length > 0) {
        logger.error(`Supabase returned 0 printers on startup — keeping config.json printers (possible misconfiguration). config.json has ${config.printers.length}.`);
      } else {
        config.printers = printers;
        printerMap = new Map(printers.map((p) => [p.printer_id, p]));
  ...
  _supabaseRefreshInterval = setInterval(refreshPrinterConfig, 5 * 60 * 1000);
});
...
async function shutdown(signal) {
  ...
  if (_supabaseRefreshInterval) {
    clearInterval(_supabaseRefreshInterval);
    _supabaseRefreshInterval = null;
  }

  try {
    await stopScheduler();
  } catch (err) {
    logger.error(`stopScheduler error: ${err.message}`);
  }
